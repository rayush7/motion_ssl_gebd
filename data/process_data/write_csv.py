...
def check_exists(row, root):
    ...
    dirname = '_'.join(['vid_' + row['youtube_id']])
...
def main_Kinetics400(mode, k400_path, f_root, csv_root='../data/kinetics400'):
    train_split_path = os.path.join(k400_path, 'kinetics_train.csv')
    val_split_path = os.path.join(k400_path, 'kinetics_validate.csv')
    test_split_path = os.path.join(k400_path, 'kinetics_train.csv')
    if not os.path.exists(csv_root): os.makedirs(csv_root)
    if mode == 'train':
        train_split = get_split(os.path.join(f_root, 'train_split'), train_split_path, 'train')
        # Filter based on Vid IDs in Kinetics GEBD TO DO-----
...
if __name__ == '__main__':
    ...
    main_Kinetics400(mode='train', # train or val or test
                      k400_path='/home/ayushrai/GEBD/data/kinetics400_256',
                      f_root='/home/ayushrai/GEBD/data/kinetics400_256/val_split/frame',
                      csv_root='/home/ayushrai/GEBD/data/kinetics400_256')