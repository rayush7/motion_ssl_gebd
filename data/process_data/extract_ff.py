...
def extract_ff_opencv(v_path, frame_root, flow_root):
    ...
    while success:
        if not os.path.isfile(os.path.join(frame_out_dir, 'image_%05d.jpg' % count)):
        ...
        #    if not os.path.isfile(os.path.join(flow_out_dir, 'flow_%05d.jpg' % count-1)):
        ...
        #     if not os.path.isfile(os.path.join(flow_out_dir, 'flow_%05d.jpg' % (count-1))): 
        #         # flow = compute_TVL1(prev_gray, image_gray)
        #         mag, ang = compute_TVL1(prev_gray, image_gray)
        #         flow_img = empty_img.copy()
        #         # flow_img[:,:,0] = mag
        #         # flow_img[:,:,1] = ang
        #         flow_img[..., 1] = 255
        #         flow_img[..., 0] = ang * 180 / np.pi / 2
        #         flow_img[..., 2] = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX)
        #         # flow_img = cv2.cvtColor(flow_img, cv.COLOR_HSV2BGR)
        #         # cv2.imwrite('flow_%05d.jpg' % (count-1),
        #         #             flow_img,
        #         #             [cv2.IMWRITE_JPEG_QUALITY, 100])
        #         # flow_img[:,:,0:2] = flow
        #         cv2.imwrite(os.path.join(flow_out_dir, 'flow_%05d.jpg' % (count-1)),
...
def main_kinetics400(v_root, frame_root, flow_root):
    ...
    for basename in ['test']:# 'val_split']:
        #if basename=='val_split':
        #    continue
        ...
        for i, j in tqdm(enumerate(v_act_root), total=len(v_act_root)):
            ...
            #flow_out_dir = os.path.join(flow_root_real,v_class)
            #if os.path.exists(out_dir) and os.path.exists(flow_out_dir): 
...
if __name__ == '__main__':
    ...
    #main_kinetics400(v_root='/home/ayushrai/kinetics400/GEBD_kinetics400_256',
    #            frame_root='/home/ayushrai/kinetics400/GEBD_kinetics400_256/test_split/frame',
    #            flow_root='/home/ayushrai/kinetics400/GEBD_kinetics400_256/test_split/flow')

    main_kinetics400(v_root='/media/ayushrai/8TB_HDD/kinetics-dataset/k400_gebd_256',
                frame_root='/media/ayushrai/8TB_HDD/kinetics-dataset/k400_gebd_256/test/frame',
                flow_root='/media/ayushrai/8TB_HDD/kinetics-dataset/k400_gebd_256/test/flow')